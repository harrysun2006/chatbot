...
function processV1Request (request, response) {
  ...
  const requestPermission = (app) => {
    app.askForPermission('To locate you', [app.SupportedPermissions.NAME, app.SupportedPermissions.DEVICE_PRECISE_LOCATION]);
  };
  ...
  const userInfo = (app) => {
    if (app.isPermissionGranted()) {
      ...
      // app.tell(`You are at ${address}, Your name :${username}`);
      return {
        username: app.getUsername().displayName,
        address: app.getDeviceLocation().address
      };
    } else {
      // app.tell('Sorry, I could not figure out who are you and where you are.');
      return {
        username: 'Guest',
        address: 'Melbourne'
      }
    ...
    'default': () => {
      ...
      if (requestSource === googleAssistantRequest) {
        let responseToUser = {
          ...
          speech: 'Hello ' + user.username, // spoken response
          text: '1.Hello ' + user.username // displayed response
        };
        ...
        let responseToUser = {
          ...
          speech: 'Hello ' + user.username, // spoken response
          text: '2.Hello ' + user.username // displayed response
        };
